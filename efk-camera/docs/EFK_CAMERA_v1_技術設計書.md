...
**本文書の設計にもとづく実装は完了しています**（`efk-camera/` 配下）。
ビルド手順は `efk-camera/README.md`、Google 側の設定は
`efk-camera/SETUP_GOOGLE_CLOUD.md` を参照してください。

### 先にお伝えする重要な制約（正直な申し送り）

この開発環境からは `dl.google.com`（Android SDK・AndroidX の配布元）へ
ネットワークアクセスが遮断されているため、**Android SDK を導入できず、
コンパイル・Unit Test・Lint・実機動作確認を行えていません。**

つまり **CI をまだ一度も緑にできていません**。これは正直にお伝えしておきます。
対策として：

- ビルド再現性を上げる仕組み（Wrapper のコミット、バージョン完全固定、
  JVM ツールチェーン固定）を最初から入れてあります（11.6.2）
- アノテーションプロセッサやコンパイラプラグインを一切使わない構成にして、
  ビルドが壊れる要因を減らしています（8.4）
- ビルドで詰まりやすい箇所を README の「ビルドでエラーが出たら」に
  先回りしてまとめてあります

**最初のビルドはつるさんの Android Studio、または GitHub Actions 上で
行っていただく必要があります。** CI が赤くなった場合は、ログを共有いただければ
こちらで直します。設計書の 11.6.3 のとおり、**CI が赤いまま完成扱いにはしません。**
...
保険として `FileOutputOptions.setFileSizeLimit(3.9e9 バイト ≒ 3.63GiB)` を設定し、
`ERROR_FILE_SIZE_LIMIT_REACHED` を受け取ったら
**録画を止めずに次のファイル（パート2）へ自動継続**します。

```
[録画セッション]
  ├ part 1 (上限で自動区切り) → YouTube「… (1/2)」
  └ part 2 (停止まで)         → YouTube「… (2/2)」
```

1080p/8Mbps なら約62分ぶんが1ファイルに入るため、
**通常の試合（20〜40分）でこの分割が起きることはありません。**
あくまで「想定外に長く回してしまったとき」の保険です。
...
### 9.1 v1 で実装した範囲（コード記述完了・ビルド検証は未実施）

- [x] KYV47 常駐サービス（カメラ・サーバ・監視）
- [x] ブラウザからの録画開始・停止
- [x] **KYV47 本体からの録画開始・停止**（LAN が落ちたときの最後の手段）
- [x] PIN / QR ペアリング + プライベートIP制限 + 総当たり対策
- [x] 状態表示（接続・電池・容量・録画状態・経過時間・温度警告）
- [x] MJPEG プレビュー（バインド失敗時は自動で無効化）
- [x] 1080p/720p 切替・ビットレート制御・映像の向き切替
- [x] ファイル上限到達時の自動パート継続 / 任意の時間分割
- [x] YouTube 再開可能アップロード（通信断から続きを再開）
- [x] Video ID 取得 + 存在確認 + 実 privacyStatus の読み戻し
- [x] 全条件を満たした場合のみの自動削除（`DeletionGuard`）
- [x] アップロード待ち管理・手動再試行・誤削除防止
- [x] 起動時の取り残しファイル回収（自動削除はしない）
- [x] 電池最適化除外の誘導・運用チェックリスト
- [x] ログの Credential マスク（`SafeLog` / `Redactor`）
- [x] GitHub Actions CI（Secret 検査 / Lint / Unit Test / ビルド）
- [x] 署名付き APK のリリース自動化（タグ `v*`）
- [x] Unit Test 8本（削除条件・再開位置・LAN判定・マスク・タイトル・容量ほか）
...
1. **自前のスクリプト `scripts/check-secrets.sh`（これがゲート）**
   外部サービスの状態に CI 全体が引きずられないよう、
   ブロックする検査は自前のスクリプトに寄せています。
   補助として gitleaks も走らせますが、そちらは **非ブロッキング**です。
2. **追跡禁止ファイルの検査** — 下記が `git ls-files` に出たら即失敗
   ...
   *.jks / *.keystore / keystore.properties / local.properties
   ...
   ```
3. **Credential パターンの検査** — `ya29.` `AIza` `1//` `-----BEGIN PRIVATE KEY-----`
   `service_account` `client_secret` など
4. **`android.util.Log` の直接呼び出しの検査** — 増えていたら失敗
   （生ログにトークンが残るのを防ぐ）
...
| 4 | Gradle キャッシュ | — |
| 5 | **Secret 混入検査**（`scripts/check-secrets.sh`） | 秘密らしきものを検出 |
...
### Q1. リポジトリの分離 → 【回答済み：A を選択いただきました】

`efk-camera` を Private で新規作成する方針で進めます。

ただし **AI 側の GitHub 権限ではリポジトリを新規作成できませんでした**
（GitHub App に repo 作成権限が無く、`403 Resource not accessible by integration`）。

そこで、

1. コードは現在の作業ブランチ `cloudcode/claude/efk-camera-v1-design-ydp96a` の
   `efk-camera/` 配下に、**そのまま専用リポジトリのルートになる形**で置いてあります
2. 空のリポジトリさえ作っていただければ、移行は1コマンドで終わります

```bash
# ① https://github.com/new で efk-camera を Private・空で作成
#    （README / .gitignore / license は全部チェックを外す）
# ② 移行スクリプトを実行
bash efk-camera/scripts/init-standalone-repo.sh
# ③ 表示されたコマンドで push
```

移行後、`cloudcode` 側の作業ブランチは削除していただいて構いません
（`main` および TAC view には一切触れていません）。
